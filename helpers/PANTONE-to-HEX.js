...
// Source: https://codebeautify.org/pantone-to-hex-converter
// const PMS = aPMS.map( c => ( m = c.match(/(\d+)/), m && m[0] )  )
const aPMS = [ 
...
// huesRGB = hues.map( h => aRGB[ PMS.indexOf(""+h) ] )
const huesRGB = [
    "FDD757",
    "FFC72C",
    "EAAA00",
...
function PMSColorMatching( C, B ) {
    ...
    for ( E = 0; E < A.length; E++ )
        A[ E ] <= B && -1 == D.indexOf( aPMS[ E ] ) && D.push( aPMS[ E ] );
    return D
}

function dec2hex( C ) {
    var B = "00";
    return C >= 0 && C <= 15 ? B = "0" + C.toString( 16 ) : C >= 16 && C <= 255 && ( B = C.toString( 16 ) ),
        B
}

function performPMS() {
    var C = $( "#viewName" ).val().split( "-" )[ 0 ];
    if ( "hex" == C ) {
        var B = $( "#h" ).val();
        6 == B.length && ( $( "#hiddenHex" ).val( B ),
            convertColor() )
    } else if ( "rgb" == C )
        $( "#hiddenHex" ).val( rgbToHex( $( "#r" ).val(), $( "#g" ).val(), $( "#b" ).val() ) ),
            convertColor();
    else if ( "hsv" == C ) {
        D = ( D = hsv2rgb( $( "#h" ).val(), $( "#s" ).val(), $( "#v" ).val() ) ).split( "," ),
            $( "#hiddenHex" ).val( rgbToHex( D[ 0 ], D[ 1 ], D[ 2 ] ) ),
            convertColor()
    } else if ( "cmyk" == C ) {
        var D;
        D = ( D = cmyk2rgb( $( "#c" ).val(), $( "#m" ).val(), $( "#y" ).val(), $( "#k" ).val() ) ).split( "," ),
            $( "#hiddenHex" ).val( rgbToHex( D[ 0 ], D[ 1 ], D[ 2 ] ) ),
            convertColor()
    }
}
...
function convertColor() {
    ...
    aww = D > 800 ? D - 400 : D,
        aww <= 400 ? bpl = 5 : aww < 500 ? bpl = 6 : aww < 600 ? bpl = 7 : aww < 800 ? bpl = 8 : aww < 1e3 ? bpl = 10 : bpl = 12;
    var A = PMSColorMatching( C, B ),
        E = "";
    if ( A.length > 0 ) {
        E = '<table border="0" cellpadding="3" style="font-family:arial;font-size:12px;font-weight:bold">';
        for ( var F = 0, r = Math.ceil( A.length / bpl ), a = 0; a < r; a++ ) {
            E += '<tr align="center">';
            for ( var e = 0; e < bpl; e++ )
                E += "<td><div>",
                    F < A.length && ( rgbcode = PMS2RGB( A[ F ] ),
                        E = E + A[ F ] + '</div><div title="' + rgbcode + '" style="background-color:#' + rgbcode + ';height:100px;width:100px;">&nbsp;',
                        F += 1 ),
                    E += "</div></td>";
            E += "</tr>"
        }
...
function generatePantoneList() {
    var C = $( "#pantoneList" ).empty();
    for ( var B in aPMS )
        $( "<option />", {
            value: aPMS[ B ],
            text: aPMS[ B ]
...
function convertPMSto( C ) {
    var B = PMS2RGB( $( "#pantoneList" ).val() );
    if ( "hex" == C ){
        $( "#h" ).val( B );
    }
    $( "#colorEffect" ).css( {
...
$( document ).ready( ( function () {
    $( "#dist" ).val( "48" ),
        $( "#colorPallete" ).colpick( {
            ...
            layout: "hex",
            submit: 0,
            onChange: function ( C, B, D, A, E ) {
                var F = $( "#viewName" ).val().split( "-" )[ 0 ];
                if ( "rgb" == F )
                    $( "#r" ).val( D.r ),
                        $( "#g" ).val( D.g ),
                        $( "#b" ).val( D.b );
                else if ( "cmyk" == F ) {
                    var r = RgbToCmyk( D.r, D.g, D.b );
                    r = r.split( "," ),
                        $( "#c" ).val( r[ 0 ] ),
                        $( "#m" ).val( r[ 1 ] ),
                        $( "#y" ).val( r[ 2 ] ),
